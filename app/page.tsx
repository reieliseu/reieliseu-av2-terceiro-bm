...
export default function Page() {
  ...
  return <main className="min-h-screen bg-background text-foreground selection:bg-accent selection:text-accent-foreground">
    ...
    {pesquisaAberta && <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/25 p-5 backdrop-blur-sm" role="presentation" onClick={() => setPesquisaAberta(false)}><div role="dialog" aria-modal="true" aria-labelledby="pesquisa-titulo" className="flex max-h-[min(720px,90vh)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-editorial" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4 border-b border-border p-5"><div><p className="font-mono text-xs uppercase tracking-[0.2em] text-primary">Pesquisa da loja</p><h2 id="pesquisa-titulo" className="mt-2 text-2xl font-semibold">Como podemos ajudar?</h2><p className="mt-1 text-sm text-muted-foreground">Pesquise livros, cursos ou temas do catálogo.</p></div><button type="button" onClick={() => setPesquisaAberta(false)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted" aria-label="Fechar pesquisa"><X /></button></div><div className="border-b border-border p-5"><label className="relative block"><span className="sr-only">Pesquisar no catálogo</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input autoFocus value={pesquisa} onChange={(event) => setPesquisa(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setPesquisaAberta(false) }} placeholder="Ex.: escuta, liderança, curso..." className="h-12 w-full rounded-xl border border-input bg-background pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" /></label></div><div className="min-h-0 flex-1 overflow-y-auto p-5"><p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Resultados</p>{catalogo.filter((produto) => `${produto.titulo} ${produto.descricao} ${produto.categoria} ${produto.formato}`.toLocaleLowerCase('pt-BR').includes(pesquisa.toLocaleLowerCase('pt-BR').trim())).slice(0, 6).map((produto) => <button key={produto.id} type="button" onClick={() => { setPesquisaAberta(false); setProdutoSelecionado(produto) }} className="mb-2 flex w-full items-start gap-3 rounded-xl p-3 text-left transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="grid size-9 shrink-0 place-items-center rounded-lg bg-muted">{produto.categoria === 'Livros' ? <BookOpen className="size-4" /> : <GraduationCap className="size-4" />}</span><span><strong className="block text-sm">{produto.titulo}</strong><span className="text-xs text-muted-foreground">{produto.categoria} · {produto.formato}</span></span></button>)}{catalogo.filter((produto) => `${produto.titulo} ${produto.descricao} ${produto.categoria} ${produto.formato}`.toLocaleLowerCase('pt-BR').includes(pesquisa.toLocaleLowerCase('pt-BR').trim())).length === 0 && <p className="rounded-xl bg-muted p-4 text-sm text-muted-foreground">Não encontramos resultados. Tente outro termo.</p>}</div></div></div>}
